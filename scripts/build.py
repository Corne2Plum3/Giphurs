    ...
    for old_name, new_name in files.items():
        ...
        try:
            ...
            if new_name.exists():
                ...
                logger.error(f'File not found: "{old_name}"')
                error_count += 1
        except Exception as err:
...
def remove_sources_inst(sources_inst_path: Path) -> int:
    ...
    try:
        ...
        logger.error(f'Failed to delete "{sources_inst_path}": {err}.')
        return 1
...
def copy_plist(src_plist: Path, dst_ufo: Path) -> int:
    ...
    try:
        ...
        logger.error(f'"{src_plist}" copy failed: {err}')
        return 1
        ...
        for subdir in font_subdir_types:        
            ...
            for target in backup_files:
                try:
                    ...
                    logger.error(f'Failed to remove "{target}": {err}')
...
if create_sources_inst(SOURCES_DIR_PATH, SOURCES_INST_DIR_PATH) != 0:
    logger.critical(f'Failed to create the UFO working directory "{SOURCES_INST_DIR_PATH}"')
    exit(1)
if empty_dir(FONTS_DIR_PATH) != 0:
    logger.critical(f'Failed to create output font directory "{FONTS_DIR_PATH}"')
    exit(1)
...
if len(UFO_FILES_LIST) == 0:
    logger.critical('No UFO file detected. Exiting...')
    logger.info('If this is the first time it happens, try again. Also, ensure the SOURCES_DIR_PATH and/or SOURCES_INST_DIR_PATH environement variable is correct if set in .env file.')
    exit(1)
...
for ufo in UFO_FILES_LIST:
    ...
    if copy_plist(SOURCES_INST_DIR_PATH / 'lib.plist', ufo / 'lib.plist') != 0:
        logger.critical(f'Failed to copy lib.plist from "{SOURCES_INST_DIR_PATH / 'lib.plist'}" into "{ufo / 'lib.plist'}"')
        exit(1)
    # use typo metrics
    ...
    if use_typo_metrics(ufo) != 0:
        logger.critical(f'Failed to set openTypeOS2Selection bit 7 in in "{ufo}"...')
        exit(1)
    # feature blocks
    if FEATURES_LOOKUPS_REF != ufo / 'features.fea':
        if copy_fea_blocks(FEATURES_LOOKUPS_REF, ufo / 'features.fea', 'feature', feature_list) != 0:
            logger.critical(f'Failed to copy features blocks from "{FEATURES_LOOKUPS_REF}" into "{ufo / 'features.fea'}"')
            exit(1)
    # lookup blocks
    if FEATURES_LOOKUPS_REF != ufo / 'features.fea':
        if copy_fea_blocks(FEATURES_LOOKUPS_REF, ufo / 'features.fea', 'lookup', lookup_list) != 0:
            logger.critical(f'Failed to copy lookups blocks from "{FEATURES_LOOKUPS_REF}" into "{ufo / 'features.fea'}"')
            exit(1)
...
# Build the fonts
# --- Main
gftools_exit_code: int = build_all_fonts(SOURCES_INST_DIR_PATH)
if gftools_exit_code != 0:
    logger.critical(f'gftools terminated with exit code {gftools_exit_code}. Stopping now.')
    exit(1)
rename_error_count_3a: int = rename_loop(
        {
...
if rename_error_count_3a > 0:
    logger.critical('Some font binaries are missing. Stopping now.')
    exit(1)
# --- Small Caps
sc_error_count: int = build_all_sc_fonts(font_dir=FONTS_DIR_PATH, font_subdir_types=FONTS_SUBDIRS_TYPES, processes_count=PROCESSES_COUNT)
if sc_error_count > 0:
    logger.critical(f'{sc_error_count} error(s) occured when building small caps ("SC") fonts. Stopping now.')
    exit(1)
logger.info('Renaming variable italic SC file...')
rename_error_count_3b: int = rename_loop({FONTS_DIR_PATH / 'variable' / f'{FONT_NAME}-ItalicSC[wght].ttf': FONTS_DIR_PATH / 'variable' / f'{FONT_NAME}SC-Italic[wght].ttf'})
if rename_error_count_3b > 0:
    logger.critical('Some font binaries are missing. Stopping now.')
    exit(1)
...
hinting_errors: int = add_hinting_all(FONTS_DIR_PATH, FONTS_SUBDIRS_TYPES, False, PROCESSES_COUNT)
if rename_error_count_3b > 0:
    logger.critical('Some error occured when adding hinting. Stopping now.')
    exit(1)
